...
/** A clock-driven scheduler that runs main-thread work inline and lets a test advance time. */
...
class ValidationAndAbuseTest {
    ...
    @Test fun `policies run in order and first rejection wins`() {
        ...
        // self-invite (first policy) wins over the always-reject second policy
    ...
    @Test fun `per-inviter rate limit blocks the over-limit send and reports retry`() {
        ...
        // window slides: after 1000ms the first send falls out, a new one is allowed
        sched.advance(1000)
        assertInstanceOf(
            InvitationManager.SendResult.Accepted::class.java,
    ...
    @Test fun `admin send bypasses the rate limiter`() {
        ...
        m.send(Invite(inviterId = a, invitedId = b))
        // would normally be limited, but admin bypasses
        assertInstanceOf(
            InvitationManager.SendResult.Accepted::class.java,
