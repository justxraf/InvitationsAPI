...
/** A store whose writes can be made to throw on demand, to exercise [StoreFailurePolicy]. */
...
class PersistenceDurabilityTest {
    ...
    @Test fun `enforceCaps drops the oldest over the inviter cap`() {
        ...
        // the two survivors are the newest (createdAt 2 and 3)
    ...
    @Test fun `a second JsonFileStore over the same locked file is rejected`(@TempDir dir: Path) {
        ...
        first.close()
        // After close the lock is released and a fresh store can open.
    ...
    @Test fun `AsyncStore applies queued writes by load() and survives close()`() {
        ...
        async.remove(a.id)
        // load() flushes first, so it must reflect every queued op.
        assertEquals(listOf(b.id), async.load().map { it.id })
        async.close()
        // close() drains and writes through to the delegate.
