...
/** Integration tests for [SqlInvitationStore] against an in-memory SQLite database. */
class SqlInvitationStoreTest {

    // A single shared in-memory connection (":memory:" databases vanish when their last connection
    // closes), so the store is built with closesConnections = false.
    ...
    @Test fun `migrate is idempotent and records the latest schema version`() {
        store() // first construction migrates
        store() // second construction must be a no-op, not throw
