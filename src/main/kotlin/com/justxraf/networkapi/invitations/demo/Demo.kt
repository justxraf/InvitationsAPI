package com.justxraf.networkapi.invitations.demo
...
/**
 * Server-free demo: the same engine powers two different invitation systems (IslandCore-style island
 * invites and SkyblockAPI-style teleport requests). Run via `./gradlew run`. The stand-in types here
 * only mirror the shape of the real plugins.
 */

// ---- Plugin A: IslandCore-style island invite ----------------------------------------------------
...
// ---- Plugin B: SkyblockAPI-style teleport request -----------------------------------------------
...
data class TeleportRequest(
    ...
    override val createdAt: Long = System.currentTimeMillis(),
    override val expiresAt: Long? = createdAt + 120_000, // teleport requests time out (120s)
) : Invitation
...
// ---- A trivial scheduler so the demo runs without a server, with on-demand timers ---------------
...
fun main() {
    ...
    scheduler.fireAllTimers() // the un-answered teleport request times out
