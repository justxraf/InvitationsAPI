...
/**
 * Encodes a flat [InvitationSerializer] `Map<String, String>` to a single string and back, so a
 * store can persist arbitrary domain fields in one text column ([SqlInvitationStore]'s `fields`)
 * without a schema per invitation type. The form is the same single-object JSON the [JsonFileStore]
 * codec produces, kept dependency-free.
 */
